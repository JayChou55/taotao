...
public class MainTest {
	...
	private static void testGateWayPay( String payKey , String paySecret){
		...
		for (int i = 0; i < 100; i++) { //每条线程生成数
			...
			String notifyResultStr = SimpleHttpUtils.httpGet("http://localhost:8082/gateway/scanPayNotify/notify/TEST_PAY_HTTP_CLIENT", notifyMap);
	...
	private static Map<String , Object> getInitRequestMap(String merchantOrderNo , String payKey , String paySecret){
        ...
        paramMap.put("returnUrl","http://localhost:8084/shop/roncooPayNotify/notify"); // 页面通知返回url
        paramMap.put("notifyUrl","http://localhost:8084/shop/roncooPayNotify/notify");// 后台消息通知Url
